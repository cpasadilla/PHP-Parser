<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Embarkation Record') }}
            </h2>
            <a href="{{ route('crew.show', $crewEmbarkation->crew_id) }}" 
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('crew-embarkations.update', $crewEmbarkation) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <select name="crew_id" id="crew_id" required 
                                    ...
                                    @foreach($crews as $crew)
                                        <option value="{{ $crew->id }}" 
                                                {{ (old('crew_id', $crewEmbarkation->crew_id) == $crew->id) ? 'selected' : '' }}>
                                            {{ $crew->full_name }} ({{ $crew->employee_id }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('crew_id')
                                ...
                                <select name="ship_id" id="ship_id" required 
                                    ...
                                    @foreach($ships as $ship)
                                        <option value="{{ $ship->id }}" 
                                                {{ (old('ship_id', $crewEmbarkation->ship_id) == $ship->id) ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <input type="date" name="embark_date" id="embark_date" 
                                       value="{{ old('embark_date', $crewEmbarkation->embark_date->format('Y-m-d')) }}" required
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                                @error('embark_date')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="date" name="disembark_date" id="disembark_date" 
                                       value="{{ old('disembark_date', $crewEmbarkation->disembark_date?->format('Y-m-d')) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
                            ...
                            <div>
                                ...
                                <input type="text" name="embark_port" id="embark_port" 
                                       value="{{ old('embark_port', $crewEmbarkation->embark_port) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                            ...
                            <div>
                                ...
                                <input type="text" name="disembark_port" id="disembark_port" 
                                       value="{{ old('disembark_port', $crewEmbarkation->disembark_port) }}"
                                       class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                        ...
                        <div class="mt-6">
                            ...
                            <textarea name="remarks" id="remarks" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                                      placeholder="Any additional notes about this embarkation...">{{ old('remarks', $crewEmbarkation->remarks) }}</textarea>
                            @error('remarks')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <script>
        ...
        // Set initial min date for disembark
        const embarkDate = document.getElementById('embark_date').value;
        if (embarkDate) {
            document.getElementById('disembark_date').min = embarkDate;
        }
    </script>
</x-app-layout>
